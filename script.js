...
function handleYes() {
  if (!runawayMode) {
    // Tease: haven't explored the No button enough yet
    showToast(YES_TEASE_MSGS[yesTeasIdx % YES_TEASE_MSGS.length], 3200);
    yesTeasIdx++;
    // Small bounce on yes button to feel playful
    yesBtn.style.transform = "scale(1.15)";
    setTimeout(() => { yesBtn.style.transform = ""; }, 200);
  } else {
    // True yes — go to celebration page
...
function enableRunawayMode() {
  ...
  // Remove old click handler to avoid double-fire; add pointer/touch listeners
  ...
  noBtn.addEventListener("touchstart", moveNoBtnRandom, { passive: true });
